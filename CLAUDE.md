...
当用户要求处理本仓库的代码、审核或合并 PR 时，先阅读 [GOVERNANCE.md](GOVERNANCE.md) 和 [MAINTAINERS.md](MAINTAINERS.md)，按维护任务执行；以下求职配置开场流程仅用于实际求职使用场景，不要因维护检出目录缺少 `config.yaml` 而启动求职面板。

遵循 [处理结束时的合并与交接](GOVERNANCE.md#处理结束时的合并与交接)：在维护者已授权的任务范围内，当前提交满足有效独立审核、必需检查、无冲突及阻塞处理要求，且当前账号有合并权限时，主动按正常流程合并并确认 `MERGED`，不要只说“审核通过”或再等项目负责人批准。用户明确要求只审不合或暂缓时不合并；缺权限或有阻塞时，在 PR 明确交接给对应维护者。禁止管理员绕过、降低保护或把自审/机器人结果计作独立批准。不创建定时提醒。
...
bosshunter ai-status        # 安全检测 AI API 连接（不显示 Key）
